...
int main() {
    ...
    psmCheck(result.diverterCommand == psm::DiverterCommand::Divert, "a 750g item commands Divert");
    psmCheck(result.diverterActual == psm::DiverterPosition::Moving, "diverter begins moving on tick 0");
    ...
    psmCheck(result.diverterActual == psm::DiverterPosition::Diverted, "diverter has settled by tick 1");
    ...
    psmCheck(result.item->zone == psm::Zone::OutputHeavy,
             "by tick 3 the settled diverter routes the item to OutputHeavy");

    result = engine.step();
    psmCheck(result.tick == 4, "fifth step reports tick 4");
